//! Checkout (URL-based): Refund and refund-status types.
//!
...
mod tests {
    ...
    fn refund_response_round_trip() {
        ...
        assert_eq!(back.refund_trx_id, resp.refund_trx_id);
        assert_eq!(back.refund_amount.as_str(), resp.refund_amount.as_str());
    }
    ...
    #[test]
    fn refund_status_response_parses_minimal() {
        let body = r#"{
            "paymentID": "TR0001",
            "trxID": "8A00ABCD",
            "refundTrxID": "8B00EFGH",
        ...
        let resp: RefundStatusResponse = serde_json::from_str(body).unwrap();
        assert_eq!(resp.refund_trx_id, "8B00EFGH");
